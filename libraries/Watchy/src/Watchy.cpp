...
GxEPD2_BW<GxEPD2_150_BN, GxEPD2_150_BN::HEIGHT> Watchy::display(
    GxEPD2_150_BN(DISPLAY_CS, DISPLAY_DC, DISPLAY_RES, DISPLAY_BUSY));
...
void Watchy::handleButtonPress() {
  ...
  // Up Button
    ...
    if (guiState == MAIN_MENU_STATE) { // increment menu index
      ...
      RTC.read(currentTime);
      int dahs = (currentTime.Hour / 3) % 4;
      int dits = currentTime.Hour % 3;

      for (int j = 0; j < dahs; j++) {
        vibMotor(200, 2);

        delay(150); // Inter-dash pause
      }

      delay(500);

      for (int j = 0; j < dits; j++) {
        vibMotor(85, 2);

        delay(125); // Inter-dot pause
      }
      return;
    }
  }
  // Down Button
    ...
    if (guiState == MAIN_MENU_STATE) { // decrement menu index
      ...
      RTC.read(currentTime);
      int dahs = (currentTime.Minute / 10);
      int dits = currentTime.Minute % 10;

      for (int j = 0; j < dahs; j++) {
        vibMotor(160, 2);

        delay(125); // Inter-dash pause
      }

      delay(1000);

      for (int j = 0; j < dits; j++) {
        (j == 4) ? vibMotor(115, 2) : (j % 2) ? vibMotor(65, 2) : vibMotor(100, 2); // 4 for the middle 5th buzz of 0-9

        (j % 2) ? delay(115) : delay(105); // Inter-dot pause
      }
      return;
    }
  }
